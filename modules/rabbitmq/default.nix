...

{
  ...
  config = mkIf config.services.rabbitmq.enable {
    ...
    launchd.user.agents.rabbitmq-server =
      { path = [ pkgs.coreutils pkgs.gnused cfg.package ];
        environment = {
          RABBITMQ_MNESIA_BASE = "${cfg.dataDir}/mnesia";
          RABBITMQ_LOGS = "-";
          RABBITMQ_LOG_BASE = "${cfg.dataDir}/logs";
          SYS_PREFIX = "";
          RABBITMQ_CONFIG_FILE = "${config_file}";
          RABBITMQ_PLUGINS_DIR = concatStringsSep ":" cfg.pluginDirs;
          RABBITMQ_ENABLED_PLUGINS_FILE = "${plugin_file}";
